...
year= 2005;Gender1="Male";Gender2="Female";country="ITA"



DEC_RES=function(start.age, second.age, open.age, Gender1,Gender2,
                 year, country){
  
  # 5) Getting the female death rates for ages >=65 in 1970 and 1990
  mx1 <- filter(Final,Year==year & Ageclass >=start.age & Gender == Gender1 & CNT == country)$mx
  mx2 <- filter(Final,Year==year & Ageclass >=start.age & Gender == Gender2 & CNT == country)$mx
  # 6) We considered 85+ as the open age interval to match the disability data
  
  
  wx1 <- filter(Final,Year==year & Ageclass >=start.age & Gender == Gender1 & CNT == country)$Prevalence
  wx2 <- filter(Final,Year==year & Ageclass >=start.age & Gender == Gender2 & CNT == country)$Prevalence
  # 6) We considered 85+ as the open age interval to match the disability data
...
DEC_RES(start.age =16,second.age = 20,
        open.age = 85,year = year, 
        Gender2=Gender2, Gender1=Gender1,country="ITA")
...
for (i in 1:length(country)) {
  ...
  Res[[cnt]] <-  DEC_RES(start.age =16,second.age = 20,
                         open.age = 85,year = year, 
                         Gender2=Gender2, Gender1=Gender1,country=cnt)
